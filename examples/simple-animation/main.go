...
func main() {
	...
	webpanim := webp.NewWebpAnimation(1062, 938, 0)
	...
	webpConfig := webp.NewWebpConfig()
	webpConfig.SetLossless(1)

	err = webpanim.AddFrame(frame1, 0, webpConfig)
	if err != nil {
		log.Fatal(err)
	}
	err = webpanim.AddFrame(frame2, 1000, webpConfig)
	if err != nil {
		log.Fatal(err)
	}
	err = webpanim.AddFrame(frame3, 2000, webpConfig)
	if err != nil {
		log.Fatal(err)
	}
	err = webpanim.AddFrame(frame4, 3000, webpConfig)
	if err != nil {
		log.Fatal(err)
	}
	err = webpanim.AddFrame(frame5, 4000, webpConfig)
	if err != nil {
		log.Fatal(err)
	}

	err = webpanim.AddFrame(nil, 5000, webpConfig)
	if err != nil {
		log.Fatal(err)
	}
